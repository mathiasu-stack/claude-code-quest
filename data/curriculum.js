...
<p>When something doesn't work the way a tutorial says it should, your first instinct should be: <em>"Has this changed?"</em> — not <em>"Am I doing something wrong?"</em> Check <code>claude --help</code>, check the changelog, then adjust. This skill transfers to every AI tool you'll ever use.</p>`,
    ...
    practicalTest: {
      ...
      scenario: 'Welcome aboard. The support team is drowning. I want you to set them up with Claude Code so they reply consistently and fast. Step 1: create a folder called `kedash-support/`, drop a 3-line README, and actually open `claude` inside it. Send me back: the commands you ran, the README contents, and a snippet from your first session — at minimum the welcome banner and one tiny exchange (say `Hi` and paste Claude\'s reply).',
      task: 'Paste the setup commands, the README content, and a snippet from your first Claude session (banner + one tiny exchange).',
      hint: 'Use sections — `## Setup`, `## README`, `## First session`. One exchange is enough.',
      minLength: 150, passThreshold: 70, xpReward: 300,
      criteria: [
        { type: 'keyword', value: ['mkdir', 'kedash-support'], description: 'Creates the project folder', weight: 2 },
        { type: 'keyword', value: ['README'], description: 'Includes a README', weight: 2 },
        { type: 'keyword', value: ['claude code', 'welcome', 'version', 'model', 'claude '], description: 'Pasted the session banner / first reply', weight: 2 },
        { type: 'keyword', value: ['## Setup', '## README', '## First session'], description: 'Used section markers', weight: 1 },
        { type: 'keyword', value: ['support', 'team', 'replies', 'responses'], description: 'README states the support purpose', weight: 1 },
      ],
      exemplar: '<p>Strong answer: <code>mkdir kedash-support/ && cd kedash-support/</code>, a 3-line README naming the support-team purpose, then <code>claude</code> opened with its welcome banner + a tiny exchange ("Hi" → its reply).</p>',
    },
  },
